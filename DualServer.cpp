...
expiryMap dnsAge[2];
//expiryMap dhcpAge;
char serviceName[] = "CnbbxServer";
char displayName[] = "Cnbbx DHCP DNS Service";
//char tempbuff[512];
//char extbuff[512];
//char logBuff[512];
char htmlTitle[256] = "";
...
const char sVersion[] = "Cnbbx DHCP DNS Server Version 1.1";
...
const char bodyStart[] = "<style>\
table{width:100%%;border-collapse:collapse;font-family:Futura,Arial,sans-serif;margin: 3em auto;}\
caption{font-size:larger;margin:1em auto}\
td,th{padding:.65em}\
th{background:#555 nonerepeat scroll 0 0;color:#000}\
...
<body><div style=\"margin:0 auto;width:75%%;\">\
<table><tr><td align=\"center\"><font size=\"5\"><b>%s</b></font></td></tr><tr><td align=\"right\"><a style=\"text-decoration:none;\" target=\"_new\" href=\"http://www.cnbbx.com/\">中国百宝箱专用DCHP和DNS服务器 By 2009-2017 &copy; Cnbbx Inc.</a></td></tr></table>";
...
void WINAPI ServiceMain(DWORD /*argc*/, TCHAR* /*argv*/[])
{
	...
	if (serviceStatusHandle)
	{
				...
				if (dhcpService)
				{
					if (network.httpConn.ready && FD_ISSET(network.httpConn.sock, &readfds))
					{
						...
						if (req)
						{
							...
							{
								sprintf(logBuff, "Accept Failed, WSAError=%d", errno);
									...
									if (dnsr.dnsp->header.ancount)
									{
										if (verbatim || cfig.dnsLogLevel >= 2)
										{
											if (dnsr.dnsType == DNS_TYPE_SOA)
												...
												//sprintf(logBuff, "%s resolved from Cache to %s (%u)", strquery(&dnsr), getResult(&dnsr), dnsAge[currentInd].size());
											else
		...
		sprintf(logBuff, "Cnbbx Server Stopped !\n");
		logMess(logBuff, 1);
...
void sendStatus(data19 *req)
{
	...
	if (cfig.dhcpRepl > t)
	{
		fp += sprintf(fp, "<caption><i>活跃的租赁</i></caption>\n");
		fp += sprintf(fp, "<tr><th>Mac地址</th><th>IP地址</th><th>租赁期满</th><th>主机名 (前20个字符)</th><th>服务器</th></tr>\n");
	}
	else
	{
		fp += sprintf(fp, "<caption><i>活跃的租赁</i></caption>\n");
		fp += sprintf(fp, "<tr><th>Mac地址</th><th>IP地址</th><th>租赁期满</th><th>主机名 (前20个字符)</th></tr>\n");
	}
	...
	for (p = dhcpCache.begin(); kRunning && p != dhcpCache.end() && fp < maxData; p++)
	{
		...
		if ((dhcpEntry = p->second) && dhcpEntry->display && dhcpEntry->expiry >= t)
		{
			...
			else
			{
				...
				strftime(tempbuff, sizeof(tempbuff), "%Y/%m/%d %H:%M:%S", ttm);
...
*/
	fp += sprintf(fp, "</table>\n<br>\n<table>\n");
	fp += sprintf(fp, "<caption><i>自由的动态租赁</i></caption>\n");
	fp += sprintf(fp, "<tr><td><b>DHCP范围</b></td><td align=\"right\"><b>提供租赁</b></td><td align=\"right\"><b>自由租赁</b></td></tr>\n");
	...
	for (char rangeInd = 0; kRunning && rangeInd < cfig.rangeCount && fp < maxData; rangeInd++)
	{
		...
		fp += sprintf(fp, "<tr><td>%s - %s</td><td align=\"right\">%i</td><td align=\"right\">%i</td></tr>\n", tempbuff, ipbuff, (ipused + ipfree), ipfree);
	}

	fp += sprintf(fp, "</table>\n<br>\n<table>\n");
	fp += sprintf(fp, "<caption><i>固定的静态租赁</i></caption>\n");
	fp += sprintf(fp, "<tr><th>Mac地址</th><th>IP地址</th><th>Mac地址</th><th>IP地址</th></tr>\n");
	...
	if (req->dnsp->header.opcode != OPCODE_STANDARD_QUERY)
	{
		switch (req->dnsp->header.opcode)
		{
			...
			case OPCODE_DYNAMIC_UPDATE:
				sprintf(logBuff, "Dynamic Update not needed/supported by Cnbbx Server");
				break;
...
MYWORD frdnmess(data5 *req)
{
	...
	if (ntohs(req->dnsp->header.qdcount) == 1)
	{
		...
		if ((dnsType == DNS_TYPE_A || dnsType == DNS_TYPE_ANY || dnsType == DNS_TYPE_AAAA || dnsType == DNS_TYPE_PTR) && !req->dnsp->header.rcode && !req->dnsp->header.tc && req->dnsp->header.ancount)
		{
			...
			if (resultFound)
			{
				req->iterBegin = dnsCache[currentInd].find(setMapName(req->tempname, req->mapname, dnsType));
...
*/
	...
	do
	{
		...
		if (dhcpService)
		{
			...
			if (!htmlTitle[0])
				sprintf(htmlTitle, "Cnbbx Server on %s", cfig.servername);
		...
		if (dnsService)
		{
			if (cfig.replication != 2)
			{
				...
				while (p != dnsCache[0].end())
				{
					if ((cache = p->second) && (cache->cType == CTYPE_SERVER_A_AUTH || cache->cType == CTYPE_SERVER_PTR_AUTH || cache->cType == CTYPE_SERVER_PTR_NAUTH))
					{
						q = p;
						p++;
						dnsCache[0].erase(q);
			...
			{
				int val = 0;
				setsockopt(network.forwConn.sock, IPPROTO_IP, IP_DONTFRAGMENT, (const char*)&val, sizeof(val));
